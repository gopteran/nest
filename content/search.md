---
title: 'Search'
draft: false
layout: 'search'
summary: 'Search the Gopteran documentation'
---

# Search Documentation

Use the search box below to find information across all Gopteran documentation.

<div id="search-container">
  <input type="search" id="search-input" placeholder="Search documentation..." />
  <div id="search-results"></div>
</div>

## Popular Searches

- [Getting Started](/docs/getting-started)
- [API Reference](/docs/api)
- [Component Overview](/docs)
- [Examples and Tutorials](/docs/examples)
- [Aerie Control Panel](/docs/aerie)
- [Carina Backend](/docs/carina)
- [Ventus Frontend](/docs/ventus)
- [Remora CLI](/docs/remora)

## Search Tips

- Use specific terms for better results
- Try different keywords if you don't find what you're looking for
- Browse by [categories](/categories) or [tags](/tags)
- Check out our [examples](/docs/examples) for practical use cases
